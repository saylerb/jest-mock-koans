...
describe("jest.fn()", () => {
  ...
  test("mock function can throw error", () => {
    const findDog = jest.fn(() => {
      throw new Error(FILL_IN_HERE);
    });
  ...
  test("should set mock implementation", () => {
    ...
    getSideDish.mockImplementation(isDiet =>
      isDiet ? "radishes" : FILL_IN_HERE
    );

    expect(getMeal(getSideDish, false)).toEqual(
      "Hot dogs with a side of poutine"
    );
  });
